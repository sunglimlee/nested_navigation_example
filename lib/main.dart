...
// 로컬 위젯 안에서 navigation 이 되도록 하는게 nested navigation, 페이지 이동에 대해서도 로컬 하나 안에서 이루어진다.
    ...
    MaterialApp(
      ...
      onGenerateRoute: (settings) {
        ...
        if (settings.name == routeHome) {
          ...
          final subRoute =
          settings.name!.substring(routePrefixDeviceSetup.length); // 앞부분 지나고 나서부터
          page = SetupFlow( // 이안에서 그냥 다 바뀌도록 하겠다는 것
...
class SetupFlowState extends State<SetupFlow> {
  final _navigatorKey = GlobalKey<NavigatorState>(); // _navigatorKey 가 있어야 navigation 을 할 수 있다.
  ...
  void _onDiscoveryComplete() {
    _navigatorKey.currentState!.pushNamed(routeDeviceSetupSelectDevicePage); // 이거때문에 onGenerate 를 실행시킨다는 거지.
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: Navigator( // 여기에 nested Navigator 가 있네.. 그것도 body 에.
          key: _navigatorKey,
          initialRoute: widget.setupPageRoute, // 밖에서 받아온 subPageRoute 이름 (setting 이 빠진것)
